from baselines.TCDF_master import TCDF
import argparse
import torch
import pandas as pd
import numpy as np
...
def runTCDF(df_data, pivm=True, kernel_size = 4, hidden_layers = 1, nrepochs = 5, learningrate = 0.01, optimizername = "Adam",
            dilation_c=4, loginterval = 250, seed = 1111, significance = 0.8, cuda = False):
    ...
    for c in columns:
        ...
        causes, causeswithdelay, realloss, scores = TCDF.findcauses(c, pivm=pivm, cuda=cuda, epochs=nrepochs,
        kernel_size=kernel_size, layers=hidden_layers+1, log_interval=loginterval,
        lr=learningrate, optimizername=optimizername,
        seed=seed, dilation_c=dilation_c, significance=significance, data=df_data)
